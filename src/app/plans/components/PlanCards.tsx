        ...
        <div className="px-1 flex rounded-4xl bg-white border border-gray-200 p-1 shadow-lg">
          {["Monthly", "Quarterly", "Yearly"].map((plan, index) => (
            ...
            >
              ...
              {plan === "Yearly" && (
                <div className="p-0.5 text-xs">Best Value</div>
              )}
            </button>
          ))}
        </div>
          ...
          (card, index) =>
            card.time === selectedPlan && (
              ...
              >
                ...
                <div className="flex font-bold text-3xl items-end justify-center">
                  {card.price}
                  <div className="font-normal text-base text-gray-500 ml-2">
                    /{card.time.toLowerCase()}
                  </div>
                </div>
                <ul className="text-left space-y-2 mt-4 mb-4">
                  {card.features.map((feature, fIndex) => (
                    <li key={fIndex} className="flex gap-2 items-center">
                      <SiTicktick className="text-[#419E5B]/70 mr-1" />
                      {feature}
                    </li>
                  ))}
                </ul>
                <button className="bg-[#419E5B] text-white px-6 py-2 rounded-full mt-auto mb-4">
                  Choose Plan
                </button>
              </div>
            )
